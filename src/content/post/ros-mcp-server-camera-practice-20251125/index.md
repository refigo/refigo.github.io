---
title: '"지금 카메라에 뭐가 보여?" ros-mcp-server로 LLM에게 ROS 로봇의 시야를 분석시켜 보았다'
...
`[이미지: 터미널에서 rosbridge_server 실행 화면]`
![[Pasted image 20251125205211.png]]
...
`[이미지: Windsurf MCP Server 설정 파일 화면]`
![[Pasted image 20251125205401.png]]

설정을 저장하고 Windsurf를 재시작합니다. Cascade(Chat) 창에 **"사용 가능한 MCP 도구가 있어?"** 라고 물어보면, `ros-mcp-server`가 연동되었음을 확인해 주고 사용할 수 있는 기능들을 나열해 줍니다.

`[이미지: ROS-MCP-Server 연동 성공 표시 대화 화면]`
![[Pasted image 20251125205653.png]]
...
[realsense launch 실행한 터미널 화면]
![[Pasted image 20251125210452.png]]
...
`[이미지: Windsurf 채팅창에서 ROS 2 System Status와 Active Topics 목록이 출력된 화면]`
![[Pasted image 20251125205857.png]]
...
살짝 틀리긴 했지만 실제로 카메라 앞에 **동그란 검정 마이크**가 있었는데, 모델은 이를 '렌즈 캡'이나 유사한 '검은 원형 물체'로 추론했습니다. 완벽하진 않지만, 이미지 토픽을 구독하여 **물체의 형태와 색상, 작업자의 행동(손을 뻗음)을 텍스트로 묘사**해 냈다는 점이 인상적입니다.

`[이미지: Windsurf가 분석한 텍스트 답변]`
![[Pasted image 20251125210604.png]]

실행된 MCP Tool의 상세 로그를 열어보면, 실제로 `subscribe_once` 함수가 호출되고 이미지가 분석된 과정을 확인할 수 있습니다.

[이미지: Cascade(Chat)에서 실행된 MCP Tool 로그 확인]
![[Pasted image 20251125210704.png]]

[이미지: 실제 RealSense가 보고 있던 화면 (비교용)]
![[Pasted image 20251125210926.png]]
